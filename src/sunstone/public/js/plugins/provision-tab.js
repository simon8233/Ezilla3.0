...
// Ezilla - Passwd - Start 
  '<div class="row">'+
     '<div class="large-8 large-centered columns graphics">'+
        '<fieldset>'+
  ...
  '<div class="row">'+
     '<div class="advanced_passwd">'+
         '<div class="large-8 large-centered columns">'+
                '<input type="password" name="user_passwd" id="user_passwd" placeholder="'+tr("New Password")+'"/>'+
                '<input type="password" name="user_passwd_confirm" id="user_passwd_confirm" placeholder="'+tr("Confirm Password")+'" />'+
        '</div>'+
     '</div>'+
  '</div>'+
//Ezilla - Passwd - end
...
//Ezilla redir Start
          '<li>'+
          '<a href"#" id="provision_spice_button" data-tooltip title="Open a Spice Information Dialog" class="has-tip tip-top">'+
            '<i class="fa fa-fw fa-2x fa-desktop"/><span style="font-size: 12px; vertical-align: middle"><br>'+tr("Spice")+'</span>'+
          '</a>'+
          '</li>'+
          '<li>'+
          '<span id="provision_spice_button_disabled" data-tooltip title="You have to boot the Virtual Machine first" class="has-tip tip-top" style="color: #999">'+
            '<i class="fa fa-fw fa-2x fa-desktop"/><span style="font-size: 12px; vertical-align: middle"><br>'+tr("Spice")+'</span>'+
          '</span>'+
          '</li>'+
          '<li>'+
          '<a href"#" id="provision_rdp_button" data-tooltip title="Open a Spice Information Dialog" class="has-tip tip-top">'+
            '<i class="fa fa-fw fa-2x fa-desktop"/><span style="font-size: 12px; vertical-align: middle"><br>'+tr("RDP")+'</span>'+
          '</a>'+
          '</li>'+
          '<li>'+
          '<span id="provision_rdp_button_disabled" data-tooltip title="You have to boot the Virtual Machine first" class="has-tip tip-top" style="color: #999">'+
            '<i class="fa fa-fw fa-2x fa-desktop"/><span style="font-size: 12px; vertical-align: middle"><br>'+tr("RDP")+'</span>'+
          '</span>'+
          '</li>'+
          '<li>'+
          '<a href"#" id="provision_ssh_button" data-tooltip title="Open a SSH Information Dialog" class="has-tip tip-top">'+
            '<i class="fa fa-fw fa-2x fa-desktop"/><span style="font-size: 12px; vertical-align: middle"><br>'+tr("SSH")+'</span>'+
          '</a>'+
          '</li>'+
          '<li>'+
          '<span id="provision_ssh_button_disabled" data-tooltip title="You have to boot the Virtual Machine first" class="has-tip tip-top" style="color: #999">'+
            '<i class="fa fa-fw fa-2x fa-desktop"/><span style="font-size: 12px; vertical-align: middle"><br>'+tr("SSH")+'</span>'+
          '</span>'+
          '</li>'+
//Ezilla redir Start
...
var provision_tab = {
  list_header: '<img src="images/Ezilla_logo_all_small.png" style="height:67px; vertical-align:top">'+
...
function RedirectPortCallback(request,response){
    ...
    $("#provision_confirm_action").html(
        '<div data-alert class="alert-box secondary radius">'+
         tr("Endpoint Information")+
         '<input type=text readonly=false id=\'connecting_textarea\' value=\"'+ 
           srv_hostname+
           ":"+response["info"]+
...
function update_provision_vm_info(data) {
  ...
  var graphic_type = data.TEMPLATE.GRAPHICS.TYPE;
...
//Ezilla redir start
      $("#provision_spice_button").hide();
      $("#provision_spice_button_disabled").hide();
      $("#provision_rdp_button").hide();
      $("#provision_rdp_button_disabled").hide();
      $("#provision_ssh_button").hide();
      $("#provision_ssh_button_disabled").hide();
//Ezilla redir end
      break;
    case "running":
...
//Ezilla redir start
      ...
      $("#provision_rdp_button_disabled").hide();
      $("#provision_ssh_button_disabled").hide();
      if (graphic_type.toLowerCase() == "vnc"){
         $("#provision_vnc_button").show();
         $("#provision_spice_button").hide();
      }else{
         $("#provision_vnc_button").hide();
         $("#provision_spice_button").show();
      }
      if (typeof(data.TEMPLATE.CONTEXT.OSTYPE) != "undefined"){
            ostype = data.TEMPLATE.CONTEXT.OSTYPE;
            if ( ostype == "WINDOWS" ){
                $("#provision_rdp_button").show();
                $("#provision_ssh_button").hide();
            }else{
                $("#provision_rdp_button").hide();
                $("#provision_ssh_button").show();
            }
      }else{
         $("#provision_rdp_button").hide();
         //$("#provision_ssh_button").show();
...
//Ezilla redir start
      $("#provision_spice_button").hide();
      $("#provision_spice_button_disabled").show();
      $("#provision_rdp_button").hide();
      $("#provision_ssh_button").hide();
      if (typeof(data.TEMPLATE.CONTEXT.OSTYPE) != "undefined"){
            ostype = data.TEMPLATE.CONTEXT.OSTYPE;
            if ( ostype == "WINDOWS" ){
                $("#provision_rdp_button_disabled").show();
                $("#provision_ssh_button_disabled").hide();
            }else{
                $("#provision_rdp_button_disabled").hide();
                $("#provision_ssh_button_disabled").show();
            }
      }else{
         $("#provision_rdp_button_disabled").hide();
         $("#provision_ssh_button_disabled").hide();
...
//Ezilla redir start
  $("#provision_info_vm").attr("vm_port", data.TEMPLATE.GRAPHICS.PORT);
//Ezilla redir end
...
$(document).ready(function(){
  ...
  if (Config.isTabEnabled(tab_name)) {
    ...
    $('#advanced_passwd').click(function(){
        $('.advanced_passwd').toggle();
        return false;
    })
...
//Ezilla Password start      
      var user_passwd = $('#user_passwd', context).val();
//Ezilla Password end     
      var template_id = $(".tabs-content .content.active .selected", context).attr("opennebula_id");
      var opennebula_ostype = $(".tabs-content .content.active .selected", context).attr("opennebula_ostype");
...
//Ezilla Password start      
      if (!$('#user_passwd',this).is(":hidden")){
         var user_passwd = $('#user_passwd', context).val();
         var user_passwd_confirm = $('#user_passwd_confirm', context).val();
         var CheckData = /[|]|{|}|<|>|'|;|&|#|"|'|!| /;
         if (user_passwd != user_passwd_confirm){
            $(".alert-box-error", context).fadeIn().html(tr("These passwords don't match. Try again"));
            return false;
         };

         if (user_passwd.length < 6){
            $(".alert-box-error", context).fadeIn().html(tr("Password must be at least 6 characters"));
            return false;
         };

         if (CheckData.test(user_passwd)){
            $(".alert-box-error", context).fadeIn().html(tr("Don't allow special characters in your password !"));
            return false;
         };
      }

      if (!$('#user_passwd',context).is(":hidden")){ // inloop vm need password initialization.
...
//Ezilla redir start
    $("#provision_info_vm").on("click", "#provision_spice_button", function(){
      var vm_id = $("#provision_info_vm").attr("vm_id");
      var vm_port = $("#provision_info_vm").attr("vm_port");
      Sunstone.runAction('Provision.redirspice', vm_id,vm_port);
    });
    $("#provision_info_vm").on("click", "#provision_rdp_button", function(){
      var vm_id = $("#provision_info_vm").attr("vm_id");
      var vm_port = 3389;
      Sunstone.runAction('Provision.redirect', vm_id,vm_port);
    });
    $("#provision_info_vm").on("click", "#provision_ssh_button", function(){
      var vm_id = $("#provision_info_vm").attr("vm_id");
      var vm_port = 22;
      Sunstone.runAction('Provision.redirect', vm_id,vm_port);
    });
